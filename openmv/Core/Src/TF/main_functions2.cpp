...
static const float INPUT_RANGE = 2.f * 3.14159265359f;
...
static const uint16_t INFERENCE_PER_CYCLE = 70;

static void handle_output(tflite::ErrorReporter* error_reporter, float x_value, float y_value);
...
int names2::setup(void)
{
    ...
    model = tflite::GetModel(conv_model);
...
static void handle_output(tflite::ErrorReporter* error_reporter, float x_value, float y_value)
{
	...
	TF_LITE_REPORT_ERROR(error_reporter, "linear: x_value: %f, y_value: %f\n", x_value, y_value);
