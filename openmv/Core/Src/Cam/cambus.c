...
//    0x20D09DE7, 0x40900C22, 0x4030040B,
	0x00C0EAFF, 0x00C0EAFF, 0x00C0EAFF,
#else
...
//    bus->i2c->Init.OwnAddress1     = 0xFE;
    bus->i2c->Init.OwnAddress1     = 0x00;
//    bus->i2c->Init.OwnAddress2     = 0xFE;
    bus->i2c->Init.OwnAddress2     = 0x00;
    #if !defined(STM32F4)
...
{
    int idx = 0;
    for (uint8_t addr=0x42; addr<=0x43; addr++) {
        if (HAL_I2C_IsDeviceReady(bus->i2c, addr << 1, 100, I2C_SCAN_TIMEOUT) == HAL_OK) {
            if (list == NULL || size == 0) {
                return (addr << 1);
            } else if (idx < size) {
                list[idx++] = (addr << 1);
