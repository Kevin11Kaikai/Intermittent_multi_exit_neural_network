...
//NORETURN
static void dma_alloc_fail(uint32_t size)
{
//    mp_raise_msg_varg(&mp_type_MemoryError,
//            MP_ERROR_TEXT
	printf("DMA buffer allocation failed while allocating %u bytes", (uint) size);
}
...
//            mp_raise_msg_varg(&mp_type_MemoryError,
//                MP_ERROR_TEXT
			printf("DMA buffer allocation failed. No DMA buffer reserved for peripheral 0x%x", (uint) periph);
            break;
    }
}
